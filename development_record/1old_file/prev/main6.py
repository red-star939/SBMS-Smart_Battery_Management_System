...
agent = Agent(state_dim=5, action_dim=2, device=device)
...
def sensor_loop():
    global manual_mode, sensor_data, app, agent, prev_soh
    ...
    while True:
        ...
        state = [
            sensor_data.get("percent", 0) / 100.0,    # SoC (0~1)
            sensor_data.get("temp", 25.0) / 50.0,    # 온도 (스케일링)
            sensor_data.get("health", 100.0) / 100.0, # SOH (0~1)
            sensor_data.get("curr", 0.0) / 10.0,     # 전류 (임의 스케일, 필요하면 조절)
            sensor_data.get("volt", 0.0) / 5.0       # 전압 (임의 스케일, 필요하면 조절)
            ]
        ...
        reward = calculate_reward(battery_percent, temperature, action, agent.prev_action, sensor_data.get("health", 100.0), prev_soh)
        prev_soh = sensor_data.get("health", 100.0)
        
        #next_state = [sensor_data.get("percent", 0) / 100.0, sensor_data.get("temp", 25.0) / 50.0]
        next_sensor_data = read_sensor_data()
        next_state = [
            next_sensor_data.get("percent", 0) / 100.0,
            next_sensor_data.get("temp", 25.0) / 50.0,
            next_sensor_data.get("health", 100.0) / 100.0,
            next_sensor_data.get("curr", 0.0) / 10.0,
            next_sensor_data.get("volt", 0.0) / 5.0
            ]
