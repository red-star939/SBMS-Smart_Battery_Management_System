def calculate_reward(battery_percent, temperature, is_charging, prev_action, health, prev_soh):
    reward = 0.0
    ...
    elif not is_charging and battery_percent > 97:      #80
        reward += 1.0  # Positive reward for stopping charge at high battery level
    elif 40 <= battery_percent <= 97:
    ...
    if prev_soh is not None:
        soh_change = abs(health - prev_soh)
        if soh_change > 2.0:
            reward -= soh_change
